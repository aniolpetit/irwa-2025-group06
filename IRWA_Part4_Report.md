...
Part 4 introduces a fully-fledged analytics layer that goes beyond counting clicks. The revamped `AnalyticsData` helper now tracks query-level events, browser fingerprints, and click attribution in memory so we can surface live insights without provisioning a database. Every POST to `/search` creates a normalized query record containing the original text, the detected browser/OS combination, a UTC timestamp, and—after the results are computed—the number of matches that were returned. This enrichment lets us highlight the zero-result rate in real time and understand which keywords are trending, while also maintaining a rolling feed of the latest user intent for debugging.

On the click side, `/doc_details` now delegates to `register_click`, which increments the traditional document counters and simultaneously attributes the interaction to the product’s brand, category, and price tier (budget, mid-range, premium). Those aggregates power brand leaderboards and a “price sensitivity” widget that reveals how often users gravitate toward each pricing bucket. Because the helper accepts either dictionaries or Pydantic `Document` objects, the same logic works regardless of whether a click originated from the processed corpus or a fallback document.

The `/dashboard` route stitches these new metrics together into a richer report. It now renders six cards in addition to the existing Altair chart:
- **Top Queries** and **Recent Searches** show normalized frequency counts and a live activity feed with browser attribution and result counts.
- **Zero-result Rate** visualizes the percentage of searches that returned no products, which is critical for spotting indexing regressions.
- **Browser Share** aggregates the user-agent families detected by `httpagentparser`, giving quick visibility into device mix.
- **Top Brands** ranks the most-clicked product manufacturers so merchandising teams can monitor exposure.
- **Price Sensitivity** uses the new price-bucket counters to show how engagement splits between budget, mid-range, and premium items.

Because all metrics are generated server-side, the dashboard can be refreshed without re-triggering heavy computations, and new widgets can be layered on top of the existing `AnalyticsData` API as future work.
