...
3. [RAG Implementation](#3-rag-implementation)
   - 3.1 [First working version (Groq baseline)](#31-first-working-version-groq-baseline)
   - 3.2 [Techniques we explored for better answers](#32-techniques-we-explored-for-better-answers)
   - 3.3 [How the iterations changed the output](#33-how-the-iterations-changed-the-output)
   - 3.4 [Conclusion](#34-conclusion)
4. [Web Analytics](#4-web-analytics)
   - 4.1 [Data collection](#41-data-collection)
   - 4.2 [Data storage model](#42-data-storage-model)
   - 4.3 [Dashboard and indicators](#43-dashboard-and-indicators)
...
![Initial Groq summary block generated with the baseline prompt.](rag-baseline.png)
...
1. **Change response format** - This really does not affect the content of the LLM summary as such, but it really improved readibility and made it much more understandable. You can compare the previous image with the original formatting against our new format in Figure 2.
...
![Redesigned Groq card showing price, rating, and availability cues.](rag-baseline-groq-skin.png)
...
![OpenAI baseline output using the original prompt structure.](rag-baseline-openai-skin.png)
...
![Groq response after the prompt rewrite highlighting trade-offs.](rag-refined-prompt-groq.png)
...
![OpenAI response after the prompt rewrite with richer explanations.](rag-refined-prompt-openai.png)
...
![`/stats` highlights total click counts per product with a simple ranking table.](stats.png)


![First fold of `/dashboard` with KPI ribbons plus request/session widgets.](analytics.png)
