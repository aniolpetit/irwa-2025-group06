...
def preprocess_query(query: str) -> List[str]:
    # Keep preprocessing consistent with corpus tokenization assumptions (lowercase + split)
    return query.lower().split()


def run_tfidf_cosine_for_queries(
    queries: List[str],
    corpus_path: str,
    top_k: int = 20
) -> Dict[str, List[Tuple[str, float]]]:
    corpus = load_processed_corpus(corpus_path)
    # Lookup from pid to full document
...
if __name__ == "__main__":
    ...
    run_tfidf_cosine_for_queries(queries, corpus_path, top_k=20)
