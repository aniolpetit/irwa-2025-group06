...
def run_custom_score_for_queries(
    queries: List[str],
    corpus_path: str,
    top_k: int = 20,
    ...
    tfidf_ranker = TFIDFRanker(index, corpus)
    custom_ranker = CustomRanker(index, corpus, tfidf_ranker=tfidf_ranker)
    ...
    for query in queries:
        ...
        ranked = custom_ranker.rank_documents(query_terms, candidate_docs)
        ...
        for i, (doc_id, score) in enumerate(ranked[:display_count]):
            ...
            rating = doc.get("average_rating", "N/A")
            discount = doc.get("discount", "N/A")
            out_of_stock = doc.get("out_of_stock", "N/A")
...
if __name__ == "__main__":
    ...
    run_custom_score_for_queries(queries, corpus_path, top_k=20)
